...
namespace SpaCyDotNet.api
{
    ...
    {
        private readonly Dictionary<string, Lexeme> _dictStr2Lex = new();
        private readonly Dictionary<BigInteger, Lexeme> _dictLong2Lex = new();
        ...
        public Lexeme this[object key]
        {
            get
            {
                switch (key)
                {
                    case string keyStr when _dictStr2Lex.TryGetValue(keyStr, out var item):
                        return item;
                    case string keyStr:
                    {
                        Lexeme lexeme;

                        if (PyVocab == null) return null;
                        using (Py.GIL())
                        {
                    ...
                    case BigInteger keyHashN:
                    {
                        if (_dictLong2Lex.TryGetValue(keyHashN, out var item))
                            return item;
                        ...
                        using (Py.GIL())
                        {
                            ...
                            lexeme = new Lexeme(dynPyObj);
                            _dictLong2Lex.Add(keyHashN, lexeme);
                        }
