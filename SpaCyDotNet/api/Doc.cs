...
namespace SpaCyDotNet.api
{
    public class Doc : IXmlSerializable
    {
        ...
        private string Text => Interop.GetString(PyDoc?.text, ref _text);

        public List<Token> Tokens => Interop.GetListFromCollection(PyDoc, ref _tokens);

        public List<Span> Sents => Interop.GetListFromGenerator(PyDoc?.sents, ref _sentences);
        ...
        public List<Span> Ents => Interop.GetListFromGenerator(PyDoc?.ents, ref _ents);
